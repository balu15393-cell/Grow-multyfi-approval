...
public final class TradeDecision {
    ...
    public static TradeTicket build(Signal s, double ltp, double capital, double utilizationPct,
        ...
        if (ltp <= 0) throw new IllegalArgumentException("Groww LTP is invalid.");
    ...
    public static TradeTicket buildExit(GrowwPortfolioClient.Position position, double ltp,
                                            String advisoryReason, String exchange) {
        if (position == null || !position.isOpen()) {
            throw new IllegalArgumentException("No open Groww MIS position was found.");
        }
        TradeTicket t = new TradeTicket();
        t.id = UUID.randomUUID().toString();
        ...
        t.advisoryEntry = position.netPrice > 0
                ? String.format("Groww MIS avg ₹%.2f", position.netPrice)
                : "Groww MIS position";
        t.ltp = ltp;
        t.orderType = "MARKET";
        t.orderPrice = null;
